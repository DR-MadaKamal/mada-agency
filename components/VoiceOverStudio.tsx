...
const VoiceOverStudio: React.FC<{
  ...
  setProject: React.Dispatch<React.SetStateAction<VoiceOverStudioProject>>;
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            const audio = await generateSpeech(project.text, project.styleInstructions, project.selectedVoice, project.aiConfig);
            ...
            await logHistory({
                type: 'audio',
                studio: 'voice_over_studio',
                content: `data:audio/wav;base64,${audio.base64}`,
                prompt: project.text,
                metadata: { voice: project.selectedVoice, style: project.styleInstructions }
            });
        } catch (err) {
    ...
    return (
        <main className="w-full max-w-5xl flex flex-col gap-4 pt-4 pb-8 mx-auto">
            <AISelector 
                config={project.aiConfig || { provider: 'google', modelId: 'gemini-1.5-flash' }} 
                onChange={(cfg) => setProject(s => ({ ...s, aiConfig: cfg }))}
                studioId="voice_over_studio"
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-4">
                    <div className="glass-card rounded-2xl p-4">
                        ...
                        <div className="min-h-[120px] flex items-center justify-center bg-black/20 rounded-lg p-4 relative overflow-hidden">
                            {project.isPlaying && (
                                <div className="absolute inset-0 flex items-center justify-center gap-1 opacity-20 pointer-events-none">
                                    {[...Array(20)].map((_, i) => (
                                        <div 
                                            key={i} 
                                            className="w-1 bg-[var(--color-accent)] waveform-bar h-12" 
                            ...
                            {!project.isLoading && project.generatedAudio && (
                                <div className="w-full flex flex-col sm:flex-row items-center gap-4 relative z-10">
                                    <button onClick={() => project.isPlaying ? stopAudio() : playAudio(project.generatedAudio as AudioFile)} className="flex-shrink-0 w-14 h-14 flex items-center justify-center rounded-full bg-[var(--color-accent)] text-white hover:bg-[var(--color-accent-dark)] transition-colors shadow-lg shadow-[var(--color-accent)]/20">
                                        {project.isPlaying ? <PauseIcon /> : <PlayIcon />}
                                    </button>
                                    <div className="flex-grow w-full flex flex-col gap-2">
                                        <p className="text-sm text-center sm:text-left text-[var(--color-text-base)] font-medium">Playback Ready</p>
