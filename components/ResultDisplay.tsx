...
interface ResultDisplayProps {
  ...
  onExpand?: (direction: 'left' | 'right' | 'top' | 'bottom') => void;
  isEditing?: boolean;
  isExpanding?: boolean;
}
...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ 
    ...
    const handleExpandAction = (dir: 'left' | 'right' | 'top' | 'bottom') => {
        if (onExpand) {
            onExpand(dir);
    ...
    return (
        <div className="w-full flex flex-col gap-4">
            ...
            <div className="w-full aspect-square bg-black/10 backdrop-blur-sm rounded-3xl flex items-center justify-center overflow-hidden relative border border-white/5 shadow-inner group">
                {(isLoading || isEditing || isExpanding) && (
                    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm">
                        <LoadingSpinner />
                        <p className="mt-4 text-[10px] font-bold text-[var(--color-accent)] uppercase tracking-[0.2em] animate-pulse">
                            {isEditing ? 'Refining Design...' : isExpanding ? 'Expanding Canvas...' : 'Generating Design...'}
                        </p>
                    </div>
                )}
                
                {!isLoading && !isEditing && !isExpanding && !imageFile && (
                ...
                {!isLoading && !isEditing && !isExpanding && imageFile && (
                     <div className="w-full h-full relative">
                        ...
                        {/* Outpainting Controls Overlay */}
                        {onExpand && (
                             <div className="absolute inset-0 pointer-events-none p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <div className="w-full h-full relative">
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); setShowExpandPanel(!showExpandPanel); }}
                                        className="absolute top-2 right-2 p-3 bg-white/10 backdrop-blur-xl border border-white/10 rounded-full text-white hover:bg-[var(--color-accent)] hover:border-[var(--color-accent)] transition-all pointer-events-auto shadow-xl"
                                        title="Expand Infinite Canvas"
                                    >
                                        <ExpandIcon />
                                    </button>

                                    {showExpandPanel && (
                                        <div className="absolute top-14 right-2 bg-black/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-4 pointer-events-auto grid grid-cols-3 gap-2 shadow-2xl animate-in zoom-in-95 duration-200">
                                            <div />
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleExpandAction('top'); }}
                                                className="w-10 h-10 rounded-xl bg-white/5 hover:bg-[var(--color-accent)] flex items-center justify-center text-white transition-all transform active:scale-90"
                                            >
                                                ↑
                                            </button>
                                            <div />
                                            ...
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleExpandAction('left'); }}
                                                className="w-10 h-10 rounded-xl bg-white/5 hover:bg-[var(--color-accent)] flex items-center justify-center text-white transition-all transform active:scale-90"
                                            >
                                                ←
                                            </button>
                                            <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-[8px] font-black uppercase text-white/40">EXP</div>
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleExpandAction('right'); }}
                                                className="w-10 h-10 rounded-xl bg-white/5 hover:bg-[var(--color-accent)] flex items-center justify-center text-white transition-all transform active:scale-90"
                                            >
                                                →
                                            </button>
                                            ...
                                            <div />
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleExpandAction('bottom'); }}
                                                className="w-10 h-10 rounded-xl bg-white/5 hover:bg-[var(--color-accent)] flex items-center justify-center text-white transition-all transform active:scale-90"
                                            >
                                                ↓
                                            </button>
            ...
            {imageFile && !isLoading && !isEditing && !isExpanding && (
